...
export default function Setores() {
  ...
  useEffect(() => {
    const dados = localStorage.getItem("setores");
    if (dados) setSetores(JSON.parse(dados));
  ...
  function salvarSetor() {
    ...
    if (editandoId) {
      setSetores(
        setores.map((setor) =>
          setor.id === editandoId ? { ...setor, nome } : setor
        )
      );
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f6f6f6", padding: "32px" }}>
      ...
      >
        ...
        <div>
          {setoresOrdenados.length === 0 && <p>Nenhum setor cadastrado.</p>}
